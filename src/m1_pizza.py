...
def run_test_draw_points_on_circle():
    ...
    # -------------------------------------------------------------------------
    # Test 4:
    title = 'DRAW_POINTS_ON_CIRCLE, test 4:  200 magenta dots.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(150, 200), 120)
    circle.fill_color = 'teal'
    draw_points_on_circle(window, circle, 200, 'magenta')
    window.close_on_mouse_click()
...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    for k in range(len(points)):
        ...
        smolcircle = rg.Circle(points[k], 10)
        smolcircle.fill_color = color
        smolcircle.attach_to(window)
    window.render()
...
def run_test_pizza():
    ...
    # Test 4:
    title = 'PIZZA test 1:  50 slices, thin (thickness=3) blue lines.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
    circle.fill_color = 'blue'
    circle.outline_thickness = 3
    pizza(window, circle, 50, 'green', 3)
    window.close_on_mouse_click()
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    for k in range(len(slices)):
        line = rg.Line(slices[k], circle.center)
        line.color = color
...
def run_test_polygon():
    ...
    # Tests 1 and 2 (on the same window):
    title = ('POLYGON tests 2 and 3:  3 segments with thick blue lines;'
             + ' 6 with medium red lines.')
    window = rg.RoseWindow(550, 400, title)

    circle = rg.Circle(rg.Point(100, 100), 80)
    circle.outline_thickness = 3
    polygon(window, circle, 10, 'teal', 10)
    window.continue_on_mouse_click()
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    Type hints:
      ...
      :type thickness:          int
    """
    poly = generate_points_on_circle(circle, number_of_segments)
    circle.attach_to(window)
    for k in range(len(poly)):
        shape = rg.Line(poly[k-1], poly[k])
        shape.color = color
...
def run_test_fancy_polygon():
    ...
    circle = rg.Circle(rg.Point(230, 145), 120)
    circle.fill_color = 'magenta'
    fancy_polygon(window, circle, 20, 7, 'lime green', 12)
    window.close_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color,
    ...
    Type hints:
      ...
      :type thickness:       int
    """
    fancy = generate_points_on_circle(circle, number_of_lines)
    circle.attach_to(window)
    for k in range(len(fancy)):
        lines = rg.Line(fancy[k], fancy[k-hops_to_next_point])
